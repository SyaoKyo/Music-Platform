...
import me.wlw.music.model.OnlineMusicList;
...
public class SheetAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        switch (itemViewType) {
            case TYPE_PROFILE:
                if (convertView == null) {
                    convertView = LayoutInflater.from(mContext).inflate(me.wlw.music.R.layout.view_holder_sheet_profile, parent, false);
            ...
            case TYPE_MUSIC_LIST:
                if (convertView == null) {
                    convertView = LayoutInflater.from(mContext).inflate(me.wlw.music.R.layout.view_holder_sheet, parent, false);
    ...
    private void getMusicListInfo(final SheetInfo sheetInfo, final ViewHolderMusicList holderMusicList) {
        if (sheetInfo.getCoverUrl() == null) {
            ...
            holderMusicList.ivCover.setImageResource(me.wlw.music.R.drawable.default_cover);
    ...
    private void parse(OnlineMusicList response, SheetInfo sheetInfo) {
        ...
        if (onlineMusics.size() >= 1) {
            sheetInfo.setMusic1(mContext.getString(me.wlw.music.R.string.song_list_item_title_1,
                    onlineMusics.get(0).getTitle(), onlineMusics.get(0).getArtist_name()));
        } else {
            sheetInfo.setMusic1("");
        }
        if (onlineMusics.size() >= 2) {
            sheetInfo.setMusic2(mContext.getString(me.wlw.music.R.string.song_list_item_title_2,
                    onlineMusics.get(1).getTitle(), onlineMusics.get(1).getArtist_name()));
        } else {
            sheetInfo.setMusic2("");
        }
        if (onlineMusics.size() >= 3) {
            sheetInfo.setMusic3(mContext.getString(me.wlw.music.R.string.song_list_item_title_3,
    ...
    private static class ViewHolderProfile {
        @Bind(me.wlw.music.R.id.tv_profile)
    ...
    private static class ViewHolderMusicList {
        ...
        @Bind(me.wlw.music.R.id.tv_music_1)
        private TextView tvMusic1;
        @Bind(me.wlw.music.R.id.tv_music_2)
        private TextView tvMusic2;
        @Bind(me.wlw.music.R.id.tv_music_3)
